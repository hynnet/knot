...
static int serialize_rrset_chunks(wire_ctx_t *wire, const knot_rrset_t *rrset,
                                  ...
                                  int chunks_count, size_t *chunks_real_sizes,
                                  int *cur_chunk)
{
	assert(wire);
	assert(rrset);
...
int changeset_serialize(const changeset_t *ch, uint8_t *dst_chunks[],
                        size_t chunk_size, int chunks_count, size_t *chunks_real_sizes,
                        int *chunks_real_count)
{
...
int changeset_deserialize(changeset_t *ch, uint8_t *src_chunks[],
                          const size_t *chunks_sizes, int chunks_count)
{
	if (ch == NULL || src_chunks == NULL || chunks_sizes == NULL || chunks_count == 0) {
		return KNOT_EINVAL;
	}
