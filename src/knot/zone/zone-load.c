 ...
 *
 * \todo Remove in the future together with src/knot/journal/old_journal.[ch] and conf_old_journalfile()
 */
static int try_old_journal(conf_t *conf, zone_t * zone, uint32_t zone_c_serial, list_t *chgs)
{
        int ret = KNOT_EOK;
        bool is_configured;
        ...
        char *jfile = conf_old_journalfile(conf, zone->name, &is_configured);
        if (jfile == NULL) {
                return KNOT_EOK;
        }
        if (is_configured) {
                log_zone_warning(zone->name, "obsolete configuration 'journal', please update your configuration");
        }
        if (!old_journal_exists(jfile)) {
                goto toj_end;
        }
        log_zone_warning(zone->name, "obsolete journal exists: %s", jfile);
        ...
        ret = old_journal_load_changesets(jfile, zone->name, &old_chgs, zone_c_serial, zone_c_serial - 1);
        if (ret != KNOT_ERANGE && ret != KNOT_ENOENT && ret != KNOT_EOK) {
                log_zone_warning(zone->name, "obsolete journal import failed (%s)", knot_strerror(ret));
                goto toj_end;
        }
        ret = KNOT_EOK;
        ...
        log_zone_info(zone->name, "obsolete journal_imported since serial %u", zone_c_serial);

        // store them to new journal
        ret = zone_changes_store(conf, zone, &old_chgs);
        if (ret != KNOT_EOK) {
                log_zone_warning(zone->name, "unable to store imported obsolete-journal changes (%s)", knot_strerror(ret));
                goto toj_end;
        }
...
int zone_load_journal(conf_t *conf, zone_t *zone, zone_contents_t *contents)
{
	...
	(void)try_old_journal(conf, zone, serial, &chgs);
